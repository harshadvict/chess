...
let white_Queen = {
    ...
    move_forward: function () {
        if (this.direction == "South") {
            ...
            if (white_Queen.X_coordinate == black_Queen.X_coordinate && white_Queen.Y_coordinate == black_Queen.Y_coordinate)
                console.log("queen will cross each other");
            else
                this.updatePosition();
        }
    ...
    // function for the movement of the queen for specified no of steps
    ...
    jumpMoveforward: function (moves) {
        if ((Number(this.X_coordinate) + Number(moves) >= 8 || Number(this.Y_coordinate) + Number(moves)) >= 8) {
            ...
            var i;
            for (i = 0; i < Number(moves); i++) {
                white_Queen.move_forward();
            }
        }
    },

    // array to log position change

    whereabouts: [],
    updatePosition: function () {
        this.whereabouts.push(this.X_coordinate + ", " + this.Y_coordinate);
        this.j++;
    },
}
...
let black_Queen = {
    ...
    jumpMoveforward: function (moves) {
        if (Number(this.X_coordinate) - Number(moves) <= -1 || Number(this.Y_coordinate) - Number(moves) <= -1) {
            console.log("warning:-invalid move queen will cross the board");
        }
        else {
            var i;
